...
describe("resolveKimiWebBaseUrl", () => {
  ...
  test("respects KIMI_PLUGIN_CC_WEB_URL override and strips trailing slashes", () => {
    expect(resolveKimiWebBaseUrl({ KIMI_PLUGIN_CC_WEB_URL: "http://127.0.0.1:8080/" })).toBe(
      "http://127.0.0.1:8080",
    );
  });
  ...
  test("accepts 127.0.0.1 loopback address", () => {
    expect(resolveKimiWebBaseUrl({ KIMI_PLUGIN_CC_WEB_URL: "http://127.0.0.1:9000" })).toBe(
      "http://127.0.0.1:9000",
    );
  });

  test("accepts localhost loopback address", () => {
    expect(resolveKimiWebBaseUrl({ KIMI_PLUGIN_CC_WEB_URL: "http://localhost:9000" })).toBe(
      "http://localhost:9000",
    );
  });

  test("accepts [::1] IPv6 loopback address", () => {
    expect(resolveKimiWebBaseUrl({ KIMI_PLUGIN_CC_WEB_URL: "http://[::1]:9000" })).toBe(
      "http://[::1]:9000",
    );
  });

  test("rejects non-loopback hostname (example.com)", () => {
    expect(() => resolveKimiWebBaseUrl({ KIMI_PLUGIN_CC_WEB_URL: "http://example.com:9000" })).toThrow(
      /loopback address/,
    );
  });

  test("rejects private network address (192.168.1.1)", () => {
    expect(() => resolveKimiWebBaseUrl({ KIMI_PLUGIN_CC_WEB_URL: "http://192.168.1.1:9000" })).toThrow(
      /loopback address/,
    );
  });
});
