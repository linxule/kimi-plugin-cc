  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        // Pre-spawn refusal is the invariant: neither the requested workspace
        // write nor an engine-owned plan/search-index side effect can occur.
        const wrote = await fileExists(path.join(workspace, TARGET_FILENAME));
        expect(
          wrote,
          `refused agent-core-v2 must NOT create ${TARGET_FILENAME}`,
        ).toBe(false);
      } finally {
        await cleanupTestPath(kimiHome);
        await cleanupTestPath(workspace);
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        // is OUTSIDE the trusted root (the workspace). Mirror production
        ...
        const escapePrompt = [
          ...
          `denied, report the denial message verbatim and stop." Set subagent_type to "coder"`,
          `exactly as production write-swarm does. Launch the swarm immediately.`,
        ].join(" ");
