...
- **Multi-model review round (Opus + Sonnet + kimi-on-0.25.0).** A HIGH bypass in the new bare-table scanner was caught and fixed: a blank line does not end a TOML table, so a `matcher = "*"` placed after a blank line still belongs to the `[[hooks]]` table — kimi-code loads it (a matcher is a schema-valid string, so whole-file validation passes) then throws at `new RegExp("*")` at execution time and silently disables the hook. The scanner now inspects the table's full TOML span (to the next header/marker/EOF) and rejects the table if a matcher appears anywhere; only `matcher` needs this (foreign keys fail kimi-code's strict schema and are already rejected upstream). Also fixed: `--check` now recommends re-adorning markers instead of a contradictory "No action needed" when installed via a bare table, and a `KIMI_PLUGIN_CC_HOST_ID`-override edge that could append a duplicate block (a table whose command byte-equals this host's expected command is now recognized as ours regardless of host-id derivation).
