...
- **[kimi-code](https://kimi.com/code/docs)** installed and authenticated. The plugin spawns `kimi -p`; set `KIMI_PLUGIN_CC_KIMI_BIN` to override the binary location. **Certified versions are exact:** v1.10.0 runs native agent-core-v2 at kimi-code **0.42.0** (legacy-v1 stays certified 0.2.0–0.41.x for an explicitly pinned binary). Any other version is refused before Kimi starts (`KIMI_CAPABILITY_NOT_CERTIFIED`) — update the plugin, or pin `KIMI_PLUGIN_CC_KIMI_BIN` to a certified binary. If your `~/.kimi-code/config.toml` turns on default plan mode (`default_plan_mode = true`, any spelling), every command refuses with `CLI_V2_PLAN_MODE_CONFIGURED` until you set it to `false` or remove it — that one is a config fix, not a `/kimi:setup` fix. See [docs/safety.md § Refusal codes](./docs/safety.md#refusal-codes-of-the-native-v2-contract-v1100).
