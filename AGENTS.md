...
- **Version**: 1.0.3 (kimi-code, subprocess transport). v0.4.x is preserved at the `v0.4.0` tag (with a `v0.4-maintenance` branch cut from that tag, once published) for users still on kimi-cli.
- **Upstream compat**: verified against `@moonshot-ai/kimi-code` 0.2.0 (GA reference), 0.4.0, and 0.5.0 (intermediate 0.3.0 covered transitively by the byte-identical 0.2.0→0.4.0 diff for surfaces we consume; 0.4.0→0.5.0 verified directly). PR #26's policy-queue refactor preserved our PreToolUse hook's runtime position; `run-prompt.ts` is byte-identical 0.2.0→0.5.0. In 0.5.0 the hook engine was relocated from `packages/agent-core/src/agent/hooks/` to `packages/agent-core/src/session/hooks/` with byte-identical file contents; the new `--auto` CLI flag is rejected when combined with `-p`, so it cannot bypass our hook-based read-only enforcement. Audit reports 31-35 (2026-05-27 for 0.4.0; marker `compat-verified-kimi-code-0.4.0`) and 36-40 (2026-05-28 for 0.5.0; marker `compat-verified-kimi-code-0.5.0`) in `.claude/kimi-code-research/reports/`. **Playbook for future audits**: [docs/upstream-compat-audit.md](docs/upstream-compat-audit.md) — when a new kimi-code release lands, run this routine before extending `KIMI_TESTED_MINORS` or tagging a new compat marker.
