...
- **Version**: 1.2.6 (kimi-code, subprocess transport). Latest: **v1.2.6** (2026-06-19) extends `KIMI_TESTED_MINORS` through kimi-code **0.18.0** and makes `/kimi:swarm`'s `--cap` a hard concurrency ceiling on 0.18+ (`KIMI_CODE_AGENT_SWARM_MAX_CONCURRENCY`). **Full release + per-version compat history → [CHANGELOG.md](./CHANGELOG.md) and [ROADMAP-TO-GA.md § Post-GA audit log](./ROADMAP-TO-GA.md#post-ga-audit-log)** (the version-by-version compat reasoning also lives in `runtime/kimi-version-probe.ts` comments). v0.4.x (kimi-cli Wire transport) is preserved at the `v0.4.0` tag — with a `v0.4-maintenance` branch cut from that tag, once published — for users still on kimi-cli.
