...
32 KB — every host loads it on every turn (`CLAUDE.md` is an `@AGENTS.md`
import, so there is one source and nothing to mirror). Release history lives in
...
- **Toolchain**: Node >= 22.5, TypeScript, **bun** for development; npm CLI only for registry authentication/publishing
...
See [ROADMAP-TO-GA.md](./ROADMAP-TO-GA.md) for the pre-GA history and the living Post-GA audit log. v1.0.0 GA shipped 2026-05-26. Remaining open items: H1 (hook fail-open runtime drift) and H5 (per-task thinking control — upstream integration needs reassessment; the plugin exposes no override). H3/H4/H8/H9 closed in v1.2.x; H7 (real-binary smoke, `bun run smoke:real`) ships and runs on manual CI dispatch — per-push wiring is gated only on a `KIMI_MODEL_API_KEY` secret, a cost choice.
