...
This document captures the routine that ran on 2026-05-27 for `@moonshot-ai/kimi-code@0.4.0` (reports 31-35 in `.claude/kimi-code-research/reports/`, commit `b67263c`, tag `compat-verified-kimi-code-0.4.0`). Repeat it whenever a new kimi-code minor or major lands. The most recent minor worked example is the 2026-08-27 0.39.0 certification (reports 111-116); the current boundary adds the 2026-08-29 exact-0.39.1 patch check and release-candidate smoke.

## Current certified boundary (2026-08-29)

The plugin is certified through `@moonshot-ai/kimi-code@0.39.1` on the forced legacy-v1 path. The 0.39.0→0.39.1 comparison is 0 bytes across all six playbook scopes: CLI prompt mode, permission, hooks, wire/session, session bootstrap/config, and v1 tools/SDK. The exact 0.39.1 temporary binary passed `bun run smoke:real` with **12 pass / 0 fail / 55 assertions in 400.68s**, including the non-vacuous fresh/resumed default-plan legacy pin, real swarm-child denial, write-worktree confinement, and out-of-root denial. The detailed patch evidence is in `.claude/kimi-code-research/daily-monitor/2026-08-29-upstream-monitor.md`.

Native agent-core-v2 remains fail-closed disabled: accepted children force `KIMI_CODE_LEGACY_FLAG=1`, truthy `KIMI_CODE_EXPERIMENTAL_FLAG` values refuse before spawn, and the plan-file final-allow ordering gap is unchanged in exact 0.39.1 and live upstream `main` (`9d2304c`). `packages/agent-core-v2/docs/Permission.md` still reserves `event.allow()` for the plan-file guard and lists listener ordering as an open design question; no upstream issue/PR or released ordering API exists yet. 0.39.0's experimental tower mode and subagent fork remain v2-only and gated behind that same defect — neither adds a new final-allow listener. Carry MoonshotAI/kimi-code#2376 as an upstream follow-up; do not use `KIMI_PLUGIN_CC_SKIP_HOOK_CHECK` as a repair path. Operator-facing (non-contract) notes from 0.39.0: the experimental Remote Control tunnel can patch `hooks` via `POST /api/v1/config` (caught fail-closed by per-spawn byte-exact verification — altered entries refuse as drift, removed ones as not-installed) and forwards the OAuth refresh token to the hard-coded relay `wss://code-rc.kimi.com`. The detailed 0.39.0 evidence is in reports 111-116 and the 2026-08-27 daily monitor; the exact-0.39.1 patch evidence is in the 2026-08-29 daily monitor.
...
|---|---|---|---|
| `kimi -p` engine + permission mode | `apps/kimi-code/src/cli/run-prompt.ts`, `experimental-v2.ts` and their tests | engine selector semantics; on the pinned v1 path, hard-coded `permission: 'auto'`; `installHeadlessHandlers` auto-approves; resumed sessions force-overridden to `'auto'`. **Load-bearing audit lesson from 0.33.0:** `run-prompt.ts` changed only comments while `experimental-v2.ts` inverted unflagged `-p` to v2, so the selector file can never be omitted from the CLI diff. | `runtime/cli-client.ts` invokes `-p`, refuses experimental selectors, and pins `KIMI_CODE_LEGACY_FLAG=1`; safety relies on the hook firing |
...
git diff "$PREV".."$NEW" -- \
  ...
  apps/kimi-code/test/cli/experimental-v2.test.ts \
