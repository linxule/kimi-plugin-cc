...
- **Human-only for now.** There is no `kimi-swarm-write` agent — auto-dispatching a write fan-out (the read-only `kimi-swarm` agent stays read-only) is deferred until `--write` is field-proven by a real-binary smoke (write-lands-in-worktree-only, out-of-workspace-denied, git-denied, user-tree-unchanged, patch-complete, worktree-cleaned). This mirrors how the read-only swarm agent followed *after* its command was proven.
