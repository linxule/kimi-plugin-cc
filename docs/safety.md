...
- **Human-only for now.** There is no `kimi-swarm-write` agent — auto-dispatching a write fan-out (the read-only `kimi-swarm` agent stays read-only) is deferred until `--write` has more real-world use. This mirrors how the read-only swarm agent followed *after* its command was proven.
