...
- **Swarm refuses without the hook, like every model-spawning command.** A swarm makes the reason especially acute: it fans out up to `--cap` subagents, so a missing hook would remove enforcement from all of them at once. `KIMI_PLUGIN_CC_SKIP_HOOK_CHECK=1` remains an explicit tests/diagnostics escape hatch, not a recovery path.
...
|---|---|---|
| `KIMI_CAPABILITY_NOT_CERTIFIED` | the probed `kimi --version` is not in the operation's exact certified set (e.g. a background auto-upgrade moved the binary past 0.42.0) | update kimi-plugin-cc to a release certifying that version, or point `KIMI_PLUGIN_CC_KIMI_BIN` at a certified binary |
