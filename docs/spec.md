...
- a minimal rescue-specific invocation prompt plus exclusions versioned in this repo
...
- `summary`: stable semantic summary of the job result; for rescue it is seeded from the prompt at creation time and overwritten on completion with the first meaningful line of the final prose output
- `phase`: nullable transient runtime telemetry for in-progress jobs (for example `queued`, `worker-running`, `turn-running`, `done`); not part of the semantic result contract
...
`rescue` has no JSON schema.

Rules:

- rescue output is pass-through prose generated by Kimi
- the plugin preserves the raw final transcript artifact verbatim, adding only a trailing newline when needed
- empty or whitespace-only final output is rendered with the fallback message `Kimi did not return a final message.`
- the job still lands as `completed` for an empty final response; the summary falls back to `Rescue did not return a final message.`
