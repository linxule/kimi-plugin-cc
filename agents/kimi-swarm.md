...
- swarm **REFUSES without the `/kimi:setup` PreToolUse hook** (unlike single-turn review, which only warns — a fan-out with no enforcement is an N-fold blast radius). If the companion refuses, surface that and tell the user to run `/kimi:setup`; do not reach for `KIMI_PLUGIN_CC_SKIP_HOOK_CHECK`
