 ...
 */
export async function hasBornHead(cwd: string): Promise<boolean> {
...
export async function createEphemeralWorktree(
  ...
  const result = await runGitAllowFailure(
    ["worktree", "add", "--detach", worktreePath, base],
    repoRoot,
  );
  if (!result.ok) {
    throw new RuntimeError(
      ...
      "swarm.worktree",
      { details: { repoRoot, worktreePath, base } },
    );
  }
}
 ...
 */
export async function captureWorktreePatch(worktreePath: string): Promise<string> {
  ...
  try {
    ...
    throw new RuntimeError(
      ...
      `git diff (worktree capture) failed: ${execError.stderr || execError.message || "unknown error"}`,
      "swarm.worktree",
...
export async function removeWorktree(repoRoot: string, worktreePath: string): Promise<void> {
  const removed = await runGitAllowFailure(
