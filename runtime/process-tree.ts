...
interface ProcessSnapshotRow extends ProcessIdentity {
  parentPid: number;
}

type ProcRowRead =
  | { status: "found"; row: ProcessSnapshotRow }
  | { status: "missing" }
  | { status: "error" };

interface ProcessIdentityBatch {
...
export interface ProcessIdentityValidation {
  ...
  /** False means at least one identity could not be checked before the deadline. */
  complete: boolean;
}

export interface ProcessIdentityValidationOptions {
  identityReader?: ProcessIdentityReader;
  timeoutMs?: number;
}

export interface ProcessTreeWaitOptions {
  identityReader?: ProcessIdentityReader;
  /** Direct child state is tracked by ChildProcess events, not a numeric pid probe. */
  isRootAlive?: () => boolean;
}
 ...
 */
export async function collectDescendantIdentities(rootPid: number): Promise<ProcessIdentity[]> {
  if (!Number.isInteger(rootPid) || rootPid <= 0 || process.platform === "win32") return [];

  const snapshot = await raceValueWithTimeout(
    collectVerifiedDescendantIdentities(rootPid),
 ...
 */
export async function revalidateProcessIdentities(
  expectedIdentities: readonly ProcessIdentity[],
  options: ProcessIdentityValidationOptions = {},
): Promise<ProcessIdentityValidation> {
  const expected = dedupeIdentities(expectedIdentities);
  if (expected.length === 0) return { identities: [], complete: true };
  ...
  if (batch === PROBE_TIMEOUT) return { identities: [], complete: false };
...
export async function revalidateProcessIdentity(
...
export async function waitForProcessTreeExit(
  ...
  while (true) {
    ...
    if (!rootAlive && targets.length === 0) return true;

    const remainingMs = deadline - Date.now();
    if (finiteTimeout && remainingMs <= 0) return false;
    ...
    if (finiteTimeout && afterProbeRemainingMs <= 0) return false;
    await sleep(Math.min(PROCESS_EXIT_POLL_MS, afterProbeRemainingMs));
  }
}

async function collectVerifiedDescendantIdentities(rootPid: number): Promise<ProcessIdentity[]> {
  ...
  if (initialRoot === undefined || candidateRows.length === 0) return [];
...
function descendantRowsFromSnapshot(
  rootPid: number,
  rows: readonly ProcessSnapshotRow[],
): ProcessSnapshotRow[] {
  const root = rows.find((row) => row.pid === rootPid);
  if (root === undefined || isQuiescentState(root.state)) return [];

  const childrenByParent = new Map<number, ProcessSnapshotRow[]>();
  for (const row of rows) {
    if (row.pid <= 0 || row.parentPid <= 0 || row.pid === row.parentPid) continue;
    const children = childrenByParent.get(row.parentPid);
    if (children === undefined) childrenByParent.set(row.parentPid, [row]);
...
async function readCurrentSnapshotRows(
  expectedRows: readonly ProcessSnapshotRow[],
): Promise<ProcessSnapshotRow[]> {
  const procPids: number[] = [];
  const psPids: number[] = [];
...
async function collectProcessSnapshotRows(): Promise<ProcessSnapshotRow[] | undefined> {
  if (process.platform === "linux") {
    const procRows = await collectProcessSnapshotRowsFromProc();
    if (procRows !== undefined) return procRows;
  }
  return collectProcessSnapshotRowsFromPs();
}

async function collectProcessSnapshotRowsFromProc(): Promise<ProcessSnapshotRow[] | undefined> {
  let entries: string[];
  try {
...
async function collectProcessSnapshotRowsFromPs(
  pids?: readonly number[],
): Promise<ProcessSnapshotRow[] | undefined> {
  const uniquePids = pids === undefined ? undefined : uniqueValidPids(pids);
  if (uniquePids !== undefined && uniquePids.length === 0) return [];

  let stdout: string;
  try {
    const result = await execFileAsync(
      "ps",
      uniquePids === undefined
...
async function readCurrentIdentityBatch(
  expected: readonly ProcessIdentity[],
): Promise<ProcessIdentityBatch> {
  if (process.platform !== "linux") {
  ...
  for (const identity of expected) {
    if (identity.startToken.startsWith("proc:")) procPids.push(identity.pid);
    else psPids.push(identity.pid);
  }
...
async function readProcessIdentityBatchFromProc(
  pids: readonly number[],
): Promise<ProcessIdentityBatch> {
  if (pids.length === 0) return { identities: new Map(), complete: true };
...
async function readProcessIdentityBatchWithPs(
  pids: readonly number[],
): Promise<ProcessIdentityBatch> {
  const uniquePids = uniqueValidPids(pids);
  if (uniquePids.length === 0) return { identities: new Map(), complete: true };

  let stdout: string;
  try {
    const result = await execFileAsync(
      "ps",
      ["-o", "pid=,pgid=,state=,lstart=", "-p", uniquePids.join(",")],
      { timeout: PROCESS_IDENTITY_PROBE_TIMEOUT_MS, maxBuffer: 256 * 1024 },
    );
    stdout = result.stdout;
  } catch (err) {
    if (isExitCode(err, 1)) return { identities: new Map(), complete: true };
...
async function readIdentityBatchWithReader(
  expected: readonly ProcessIdentity[],
  identityReader: ProcessIdentityReader,
): Promise<ProcessIdentityBatch> {
  const outcomes = await Promise.all(
  ...
  for (const outcome of outcomes) {
    ...
    if (outcome.identity !== undefined) identities.set(outcome.identity.pid, outcome.identity);
  }
  return { identities, complete };
}

async function readProcRow(pid: number): Promise<ProcRowRead> {
  let stat: string;
  try {
  ...
  if (openParen < 1 || closeParen < openParen) return { status: "error" };
...
function parsePsSnapshotRows(stdout: string): ProcessSnapshotRow[] {
...
function mergeIdentityBatches(
  first: ProcessIdentityBatch,
  second: ProcessIdentityBatch,
...
function toProcessIdentity(row: ProcessSnapshotRow): ProcessIdentity {
  return {
    pid: row.pid,
...
function dedupeIdentities(identities: readonly ProcessIdentity[]): ProcessIdentity[] {
  const seen = new Set<string>();
  const deduped: ProcessIdentity[] = [];
...
function uniqueValidPids(pids: readonly number[]): number[] {
  return [...new Set(pids.filter((pid) => Number.isInteger(pid) && pid > 0))];
}

function psStartToken(value: string): string {
  return `ps:${value.replace(/\s+/g, " ").trim()}`;
}

function isQuiescentState(state: string): boolean {
...
async function raceValueWithTimeout<T>(
  promise: Promise<T>,
  timeoutMs: number,
