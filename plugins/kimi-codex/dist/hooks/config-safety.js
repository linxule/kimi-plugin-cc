...
const VERSION_SENSITIVE_HOOK_EVENTS = new Map([
    ["PermissionRequest", 8],
    ["PermissionResult", 8],
    ["Interrupt", 14],
]);
export function kimiConfigLockPath(configPath) {
 ...
 */
export async function withKimiConfigLock(configPath, operation, options = {}) {
    ...
    for (;;) {
        try {
            ...
            if (error.code !== "EEXIST") {
                if (error instanceof RuntimeError)
                    throw error;
                throw new RuntimeError("SETUP_CONFIG_LOCK_FAILED", `Failed to acquire setup lock ${lockPath}: ${error.message}`, "setup.config-lock", error instanceof Error
        ...
        if (Date.now() >= deadline) {
            ...
            throw new RuntimeError("SETUP_CONFIG_LOCK_TIMEOUT", `Timed out after ${waitMs}ms waiting for setup lock ${lockPath} (${ownerDetail}). Another setup may still be running; retry after it finishes or remove the lock only if that process is gone.`, "setup.config-lock", { details: { configPath, lockPath, waitMs, owner: lastOwner } });
        }
 ...
 */
async function createPrivateLockFile(lockPath, owner) {
    const candidatePath = `${lockPath}.candidate.${owner.pid}.${owner.token}`;
    const handle = await open(candidatePath, fsConstants.O_WRONLY |
        fsConstants.O_CREAT |
        fsConstants.O_EXCL |
...
async function readLockSnapshot(lockPath) {
    ...
    if (!before.isFile())
        throw unsafeLockPath(lockPath, describeFileType(before));
    let handle;
    try {
        handle = await open(lockPath, fsConstants.O_RDONLY |
    ...
    try {
        const stats = await handle.stat({ bigint: true });
        if (!stats.isFile())
            throw unsafeLockPath(lockPath, describeFileType(stats));
        ...
        const metadataTruncated = stats.size > BigInt(KIMI_CONFIG_LOCK_METADATA_MAX_BYTES) ||
...
function lockIdentity(device, inode) {
...
function unsafeLockPath(lockPath, type) {
    return new RuntimeError("SETUP_CONFIG_LOCK_UNSAFE", `Refusing setup lock ${lockPath}: the path is a ${type}, not a regular private lock file. Remove it manually after verifying its origin; setup will not follow or read it.`, "setup.config-lock", { details: { lockPath, type } });
}
...
function isStaleLock(snapshot, staleMs) {
...
async function tryRecoverStaleLock(lockPath, observed, staleMs) {
    ...
    try {
        ...
        if (current !== null &&
            ...
            if (!sameObservedLock(claimed, observed)) {
                throw new RuntimeError("SETUP_CONFIG_LOCK_RECOVERY_RACE", `Setup lock ${lockPath} changed identity while stale recovery claimed it; the claimed file was preserved at ${quarantinePath} instead of being deleted.`, "setup.config-lock", { details: { lockPath, quarantinePath, observed: observed.identity, claimed: claimed?.identity } });
            }
...
async function removeStaleRecoveryLease(recoveryPath, observed) {
    const quarantinePath = `${recoveryPath}.stale.${process.pid}.${randomBytes(16).toString("hex")}`;
    try {
    ...
    const claimed = await readLockSnapshot(quarantinePath);
    if (!sameObservedLock(claimed, observed)) {
        throw new RuntimeError("SETUP_CONFIG_LOCK_RECOVERY_RACE", `Recovery lease ${recoveryPath} changed identity while being claimed; the claimed file was preserved at ${quarantinePath} instead of being deleted.`, "setup.config-lock", {
            details: {
                recoveryPath,
                quarantinePath,
...
async function assertOwnedLock(lockPath, acquired, token) {
    const current = await readLockSnapshot(lockPath);
    if (current === null ||
...
function lockOwnershipLost(lockPath, expectedToken, current) {
    return new RuntimeError("SETUP_CONFIG_LOCK_OWNERSHIP_LOST", `Setup lock ${lockPath} changed before the protected operation began; refusing to enter the config transaction.`, "setup.config-lock", {
        details: {
            lockPath,
            expectedToken,
...
async function releaseOwnedLock(lockPath, acquired, token) {
    const current = await readLockSnapshot(lockPath);
    if (current === null ||
        current.identity !== acquired.identity ||
        current.owner?.token !== token)
    ...
    if (claimed === null ||
        ...
        throw new RuntimeError("SETUP_CONFIG_LOCK_RELEASE_RACE", `Setup lock ${lockPath} changed identity during release; the claimed file was preserved at ${releasePath}.`, "setup.config-lock", { details: { lockPath, releasePath, expectedIdentity: acquired.identity, claimedIdentity: claimed?.identity } });
    }
 ...
 */
export function normalizeTopLevelInlineHooks(contents, lineEnding) {
    let config;
    try {
        config = parseToml(contents);
    }
    catch {
...
function findTopLevelHooksAssignment(contents) {
...
function renderCanonicalHookTable(entry, lineEnding) {
    const lines = ["[[hooks]]"];
    for (const field of ["event", "matcher", "command", "timeout"]) {
    ...
    if (Object.keys(entry).some((field) => !HOOK_FIELDS.has(field)))
        return null;
 ...
 */
export function validateKimiHookSet(contents, version) {
...
export async function validateKimiHookSetForEnvironment(contents, env) {
    const inspection = inspectKimiHookSet(contents);
    ...
    if (probe.kind === "failed") {
        return invalidHookSet(`cannot verify version-sensitive event ${JSON.stringify(first.event)} because kimi-code version detection failed (${probe.reason})`, first.entry, first.line);
    }
    return validateVersionSensitiveEvents(inspection.versionSensitiveEvents, probe);
}
function inspectKimiHookSet(contents) {
    let config;
    try {
        config = parseToml(contents);
    }
        ...
        return {
            validation: invalidHookSet(`TOML parse failed: ${error instanceof Error ? error.message : String(error)}`, 0, line, "parse"),
            versionSensitiveEvents: [],
        };
    }
    const versionSensitiveEvents = [];
    const hooks = config["hooks"];
    if (hooks === undefined)
    ...
    for (let offset = 0; offset < hooks.length; offset += 1) {
        ...
        if (!result.valid)
            return { validation: result, versionSensitiveEvents };
    }
    return { validation: { valid: true }, versionSensitiveEvents };
}
function validateHookEntry(entry, index, line, versionSensitiveEvents) {
    for (const field of Object.keys(entry)) {
        if (!HOOK_FIELDS.has(field)) {
            return invalidHookSet(`unknown field ${JSON.stringify(field)} (allowed: event, matcher, command, timeout)`, index, line);
        }
    }
    const event = entry["event"];
    if (event === undefined)
        return invalidHookSet("missing required field `event`", index, line);
    if (typeof event !== "string")
        return invalidHookSet("`event` must be a string", index, line);
    ...
    if (command === undefined)
        return invalidHookSet("missing required field `command`", index, line);
...
function validateVersionSensitiveEvents(events, version) {
    if (events.length === 0)
        return { valid: true };
    ...
    for (const event of events) {
        if (version.minor < event.minimumMinor) {
            return invalidHookSet(`event ${JSON.stringify(event.event)} requires kimi-code >= 0.${event.minimumMinor}, but the installed version is 0.${version.minor}`, event.entry, event.line);
        }
    }
    return { valid: true };
}
function invalidHookSet(reason, entry, line, failureKind = "schema") {
