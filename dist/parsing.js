    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        switch (token) {
            ...
            case "--model": {
                ...
                if (value.startsWith("-")) {
                    throw new RuntimeError("INVALID_ARGS", `${token} value cannot start with '-'; pass a model name`, "swarm.parse");
                }
                model = value;
                index += 1;
                break;
            ...
            case "--budget": {
                ...
                if (!value || value.startsWith("-")) {
                    throw new RuntimeError("INVALID_ARGS", "--budget requires a duration (e.g. 30m, 1h, 90s).", "swarm.parse");
                }
            ...
            default:
                ...
                if (looksLikeFlag(token)) {
                    throw new RuntimeError("INVALID_ARGS", `Unknown flag ${token} for swarm. Supported flags: ${SWARM_SUPPORTED_FLAGS}. Use \`--\` before flag-shaped objective text to pass it through.`, "swarm.parse");
                }
                trailingTokens = argv.slice(index);
                index = argv.length;
                break;
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        switch (token) {
            ...
            case "--model": {
                ...
                if (value.startsWith("-")) {
                    throw new RuntimeError("INVALID_ARGS", `${token} value cannot start with '-'; pass a model name`, "args.parse");
                }
                model = value;
                index += 1;
                break;
