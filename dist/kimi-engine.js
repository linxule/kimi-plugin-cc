 ...
 */
export function assertNoUnsafeExperimentalSelector(env, stage = "kimi-engine.plan") {
...
export function assertExecutionPlanMatchesSpawn(plan, command, prefixArgs, env = {}) {
    ...
    if (plan.command !== command || !sameStrings(plan.prefixArgs, prefixArgs)) {
        throw new RuntimeError("KIMI_EXECUTION_PLAN_MISMATCH", "Refusing to spawn kimi because the command tuple differs from the certified execution plan.", "cli-client.pre-spawn", {
            details: {
...
export function persistedExecutionPlanFields(plan) {
    return {
        operation_kind: plan.operationKind,
        intended_engine: plan.intendedEngine,
...
export function executionPlanFromPersisted(fields) {
    if (fields.operation_kind === null ||
        fields.intended_engine === null ||
        fields.kimi_command === null ||
        fields.kimi_prefix_args === null ||
        fields.plan_certification === null) {
        throw new RuntimeError("KIMI_EXECUTION_PLAN_MISSING", "This job predates durable engine provenance and cannot be used as a subprocess execution plan.", "kimi-engine.persisted-plan");
    }
    ...
    if (!isPlanCertification(fields.plan_certification)) {
        throw invalidPersistedPlan(`unknown plan_certification ${JSON.stringify(fields.plan_certification)}`);
    }
 ...
 */
export async function reconcileHistoricalJobProvenance(store, job) {
    if (job.observed_engine !== null && job.operation_kind !== null)
        return job;
    if (!job.stream_log_path)
        return job;
    try {
        const info = await stat(job.stream_log_path);
        if (!info.isFile() || info.size > MAX_FORENSIC_LOG_BYTES)
            return job;
        const evidence = classifyHistoricalEngineProvenance(await readFile(job.stream_log_path, "utf8"));
        if (evidence.conflict)
            return job;
        return (store.backfillHistoricalProvenance(job.job_id, {
            operation_kind: evidence.operationKind,
            observed_engine: evidence.observedEngine,
            kimi_version: evidence.kimiVersion,
            system_version: evidence.observedEngine === "native-v2" ? evidence.kimiVersion : null,
        }) ?? job);
    }
    catch {
        return job;
...
export async function assertPersistedResumeLineage(store, plan, resumeSessionId, operationKind) {
    if (plan.resumedFromJobId === null) {
        throw new RuntimeError("KIMI_SESSION_LINEAGE_UNKNOWN", `Refusing to resume session ${resumeSessionId}: the persisted job carries a session id but records no source job, so its lineage cannot be established. Start a fresh session.`, `${operationKind}.resume`, {
            details: {
                refusal_kind: "session-lineage-unknown",
                retryable_after_setup: false,
                source_job_id: null,
 ...
 */
export function classifyHistoricalEngineProvenance(contents) {
    let sawLegacy = false;
    let sawNative = false;
    ...
    for (const rawLine of contents.split(/\r?\n/)) {
        ...
        if (entry.event === "record" ||
            entry.event === "session_announce" ||
            entry.event === "goal_summary" ||
            (entry.event === "exit" && entry.exit_code === 0)) {
            sawRunEvidence = true;
        }
        const protocolVersions = entry.type === "metadata" && typeof entry.protocol_version === "string"
...
function assertExecutionPlanShape(plan) {
    ...
    if (typeof candidate.certification !== "string" ||
    ...
    if (!Array.isArray(candidate.prefixArgs) ||
    ...
    if (candidate.resumedFromJobId !== null &&
...
function invalidPersistedPlan(message, cause) {
    return new RuntimeError("KIMI_EXECUTION_PLAN_INVALID", `Persisted kimi execution plan is invalid: ${message}.`, "kimi-engine.persisted-plan", cause instanceof Error ? { cause } : undefined);
}
function sameStrings(left, right) {
    return left.length === right.length && left.every((value, index) => value === right[index]);
}
async function resolveExactExecutable(command, cwd, env) {
    const candidates = [];
    if (command.includes("/") || command.includes("\\")) {
        ...
        for (const entry of searchPath.split(path.delimiter)) {
            const directory = entry.length === 0 ? cwd : path.isAbsolute(entry) ? entry : path.resolve(cwd, entry);
...
function isKimiEngine(value) {
    return value === "legacy-v1" || value === "native-v2";
}
function isPlanCertification(value) {
