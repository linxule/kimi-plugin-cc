...
OUTPUT=$(./scripts/companion.sh task rescue "$PROMPT" 2>&1) && EXIT_CODE=0 || EXIT_CODE=$?

# 1. Rescue must complete cleanly (companion exit code 0).
if [ "$EXIT_CODE" -ne 0 ]; then
  echo "FAIL: companion exited with code $EXIT_CODE" >&2
  echo "$OUTPUT" >&2
  exit 1
fi
